...
const Folder: React.FC<FolderProps> = ({ color = '#5227FF', size = 1, items = [], className = '' }) => {
  ...
  const getOpenTransform = (index: number) => {
    // Neat, symmetric fan: two side papers at equal height with mirrored tilt,
    // the front (title) raised straight up between them so nothing is covered.
    if (index === 0) return 'translate(-118%, -58%) rotate(-12deg)';
    if (index === 1) return 'translate(20%, -58%) rotate(12deg)';
    if (index === 2) return 'translate(-50%, -108%) rotate(0deg)';
    return '';
  };
